...
def do_experiment(agent_id:int, envs:list[BM], Agt:Agent, habituation:bool, n_episodes:int, table_width:int):
    ...
    for ep in range(n_episodes):
        ...
        while True:
            action = agent.step(observation)
            observation = env2.step(action)
            agent.update(observation)
            if observation["done"]:
                break
        print(f"        agent {agent_id} episode {ep} finished with {env2.step_count} steps {envs[1].__name__} {Agt.__name__} {table_width} {alpha} {gamma} {policy['e']} goal1={goal_index} goal2={goal_index2}")    
    return agent, env, env2
...
if __name__ == "__main__":
    ...
    for envs, Agt, table_width, habituation in parallel_args:
        ...
        exp_name = f"Meta_{envs[0].__name__}to{envs[1].__name__}_{Agt.__name__}_{'habituation' if habituation else 'no_habituation'}_n{n_agents}_ne{n_episodes}_a{alpha}_g{gamma}_p{policy['name']}{policy['e']}"
        ...
        if save_pickle:
            ...
            with open(f"{savedir2}/{path_name}.pickle", "wb") as f:
                tds = [(bm.step_count_list, bm2.step_count_list) for _, bm, bm2 in res]
                pickle.dump(tds, f)
